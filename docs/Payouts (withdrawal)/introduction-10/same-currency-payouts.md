...
- Only payouts with the explicit status  `failed` should be retried. If you receive a `Timeout Message` it doesn't mean the transaction has failed. Kindly re-query using this [link](https://docs.fincra.com/reference/get-payout-by-customer-reference) before you retry.
- `bankCode` Lists can be found [here](/docs/payment-scheme#bank-codes)
