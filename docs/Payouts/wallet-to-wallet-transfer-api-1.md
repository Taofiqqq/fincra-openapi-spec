---
title: Fincra Balance Transfer API
excerpt: ''
deprecated: false
hidden: false
metadata:
...
Fincra transfers can only be made from one balance to another if their base currencies are the same i.e You can only make a payout from your NGN balance to another NGN balance and not from your NGN balance to a EUR balance.

Webhooks are not sent for these transfers, you are admonished to make use of the [response object](/docs/wallet-to-wallet-transfer-api-1#wallet-to-wallet-response-object)you get when you make a transfer from your balance to another balance.
...
Take note of the following in the request body:
`customerReference` represents the unique identifier of the transaction on your system.