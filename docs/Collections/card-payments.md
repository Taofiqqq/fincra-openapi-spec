...
```json
{
  "reference": "9090JNKI", //must be 8 letters
  "card": {
...
If you receive a response with the status `processing` and auth model , this indicates that an authorization step is required. The type of authentication required for the card transaction would then be determined by the auth model returned in the data object.
...
PIN authorization requires that you collect the required fields (such as the card's PIN). As can be seen in the  response provided below, we would return a response with the status as `processing` and `auth_model` set to PIN 
...
To authorise this charge, you must then call the authorise endpoint and provide the PIN(For Pin authorization):
...
Then you should receive a response like:
...
### Handling OTP authorization

When the status in the data object is `processing` and the `auth_model` is OTP, it means that an OTP has been sent to the phone number or email address associated with the customer's bank account, and you must make a request to the authorised charge endpoint using the OTP that was sent to the customer.
...
All you have to do with 3DS authorization is redirect your user to the specified redirect URL (data.authorization.redirect). The user will then complete the authorization with their bank, and we will redirect back to the redirect url you specified earlier when initiating the charge.
...
AVS authorization requires that you collect the required fields (such as the customer's address). As can be seen in the  response provided below, we would return a response with the status as `processing` and `auth_model` set to `AVS `
...
To authorise this charge, you must then call the [authorize endpoint](/reference/authorize-a-charge) Customer's address(For AVS): 
