...
excerpt: >-
  This API lets you create NGN and MCY virtual accounts. You’ll need to complete
  currency-specific fields as well as shared fields applicable to all
  currencies. E.g `firstName` and` lastName` are applicable to both NGN and MCY
  account requests while `meansofID` is applicable to only MCY accounts.
api:
